...
def default_books() -> list[dict[str, Any]]:
	book_details = [
		{
...
def setup_database(db_file: str) -> Tuple[sqlite3.Cursor, sqlite3.Connection]:
    con = sqlite3.connect(db_file, check_same_thread=False)  # Prevents threading issues
    c = con.cursor()
    c.execute("""
		CREATE TABLE
			IF NOT EXISTS books(
				...
				CONSTRAINT uc_book UNIQUE (author, country, imageLink, language, link, pages, title, year)
			)""")
    con.commit()
